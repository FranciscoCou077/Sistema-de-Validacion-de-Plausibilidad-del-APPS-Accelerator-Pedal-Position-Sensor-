...
const int ADC_MAX_CH2 = 2700;   // corresponde a ~2.5V en canal 2 (rango mas chico)
...
// Control de impresion: el muestreo de seguridad sigue siendo cada 1ms,
// pero solo imprimimos al Serial cada 200ms para que sea legible por humanos.
const unsigned long INTERVALO_IMPRESION_MS = 200;
...
void loop() {
  ...
  // Solo imprimimos cada 200ms para que sea legible; la deteccion de seguridad
  // de arriba SIGUE corriendo cada 1ms sin importar esto.
